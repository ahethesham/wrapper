...
class basic_token_interface{
    public:
        ...
        using token_handler = std::function< std::shared_ptr<object_interface> ( std::shared_ptr<tokenizer_interface>) > ;

        virtual bool compare( char ch) = 0;

        //virtual std::shared_ptr<object_interface> cb(tokenizer_interface &  tokenizer) = 0;
        virtual std::shared_ptr<object_interface> cb(std::shared_ptr<tokenizer_interface> tokenizer) = 0;
