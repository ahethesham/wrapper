...
class http_headers_v1::impl{
    ...
    public:
        ...
        void at_eof(buffer_v1 * buffer){
            if(cb_(buffer))
                throw std::runtime_error("Incomplete http headers received ");
            return ;
        }
        ...
        std::string serialize(basic_formatter_interface & formatter){
            ...
            formatter.header_post_processor(res);
            return res;
        }
...
//copy constructor yet to be implemented 
http_headers_v1::http_headers_v1(http_headers_v1 & obj) : impl_(new impl()){
    for(auto itr : obj.impl_->get()){
        impl_->set(itr.first , itr.second) ;
    } 
}
...
std::string http_headers_v1::serialize(){
    return impl_->serialize();
}

http_headers_v1::self_type & http_headers_v1::clear(){
    impl_->clear();
    return *this;
}
