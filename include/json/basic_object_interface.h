...
class basic_object_interface : public basic_parser_interface<buffer_v1>{
    public:
         template<typename T>
          ...
          template<typename T>
              static std::shared_ptr<T> build(std::shared_ptr<basic_tokenizer_interface<basic_object_interface>> tokenizer){
                  return std::make_shared<T>(tokenizer);
              }
          ...
          virtual void parse(std::shared_ptr<basic_tokenizer_interface<basic_object_interface>> tokenizer) = 0;
          //parser methods
