...
json_object_v1::json_object_v1(json_tokenizer &  tokenizer) : buffer_(nullptr) , storage_(std::make_shared<std::map<std::string , std::shared_ptr<basic_object_interface>>>()){    
    this->parse(tokenizer);
}

json_object_v1::json_object_v1(std::shared_ptr<basic_tokenizer_interface<basic_object_interface>> tokenizer) : buffer_(nullptr) , storage_(std::make_shared<std::map<std::string , std::shared_ptr<basic_object_interface>>>()){
    parse(tokenizer);
}

json_object_v1::json_object_v1() : buffer_(nullptr) , storage_(std::make_shared<std::map<std::string , std::shared_ptr<basic_object_interface>>>()){}
 ...
 * Copy construct
 */
json_object_v1::json_object_v1(json_object_v1 & obj) : buffer_(nullptr) , storage_(std::make_shared<std::map<std::string , std::shared_ptr<basic_object_interface>>>()){
    for(auto itr : *obj.storage_){
        (*storage_)[itr.first] = itr.second;
    }
}

json_object_v1::json_object_v1(const char * input) : buffer_(nullptr) , storage_(std::make_shared<std::map<std::string , std::shared_ptr<basic_object_interface>>>()){
    auto tokenizer = tokenizer_builder<json_tokenizer>(input);
    parse(tokenizer);
}

std::map<std::string , std::shared_ptr<basic_object_interface>> & json_object_v1::value(){
    return *storage_;
}
...
void json_object_v1::parse(std::shared_ptr<basic_tokenizer_interface<basic_object_interface>> tokenizer){
    assert(tokenizer->getNext()->compare('{'));
    while(tokenizer->hasNext()){
        if(tokenizer->peek_next_valid_char() == '}'){
            tokenizer->get_next_valid_char();
...
json_object_v1 & json_object_v1::push(std::string key , std::shared_ptr<basic_object_interface> object){
    storage_->insert({key , object});
    return *this;
}
...
void json_object_v1::parse(buffer_type * buffer){
    ...
    std::shared_ptr<json_tokenizer >tokenizer = tokenizer_builder<json_tokenizer>(buffer->data + buffer->head);
    parse(tokenizer);
    return ;
}
...
int json_object_v1::size(){
    int total = 0;
    for(auto itr : *storage_){
        total += itr.first.size();
        total += 2; // for "" around key as string
        total += 2; // for : and space
        total += itr.second->size();
        total ++; // for ,
    }
    if(total > 0)total-- ; // for removing , after last object
    total += 2; // for {}
    return total;
}
