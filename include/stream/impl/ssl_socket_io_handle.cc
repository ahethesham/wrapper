    ...
    public:
        impl(){
            ...
            assert(detail::socket_wrapper(network_type , communication_type , protocol_type , [&](int fd){
                        fd_ = fd;
                        return fd <= 0 ? -1 : SSL_set_fd(ssl_fd_ , fd) == 1 ? 0 : 1;
            }) == 0);
        }
...
template<auto network ,
    auto communication ,
    auto protocol >
ssl_socket_io_handle<network , communication , protocol>::fd_type ssl_socket_io_handle<network , communication , protocol >::get(){
    return impl_->get();
}
