...
class https_connection_v1::impl{
    public:
        impl() : response_(nullptr), writer_(nullptr) , reader_(nullptr) , socket_(nullptr) , response_builder_(nullptr) , request_builder_(nullptr){
        ...
        impl(std::shared_ptr<ssl_socket_io_handle_interface> socket , std::shared_ptr<basic_endpoint_interface> endpoint)
            : socket_(socket) , endpoint_(endpoint) , response_(nullptr) , writer_(nullptr) , reader_(nullptr), response_builder_(nullptr) , request_builder_(nullptr) {}
        ...
        std::shared_ptr<basic_http_response_interface> send(std::shared_ptr<basic_http_request_interface> req){
            writer_->write(*req);
            auto res = response_builder_->build();
            reader_->read(*res);
            return res;
        }
        ...
        void connect(){
            ...
            while(itr->has_next()){
                ...
                if(detail::connect_wrapper(socket_->get_lowest_level_fd() , addr_info  , [&](int rc){
                    ...
                    if(rc != 0){
                        LOG_CRITICAL << "connection failed error = " << strerror(errno) << endl;
                    }
    ...
    private:
        ...
        std::shared_ptr<basic_request_builder_interface<basic_http_request_interface>> request_builder_;
...
https_connection_v1::https_connection_v1() : impl_(std::make_shared<impl>()){ 
     LOG_WARN << "default constrcutor for " << __func__ << "is deprecated make sure we inject socket and endpoint objects explicitly" << endl;
}
...
std::shared_ptr<basic_http_response_interface> https_connection_v1::send(std::shared_ptr<basic_http_request_interface> req){
    return impl_->send(req);
}
https_connection_v1 & https_connection_v1::send(basic_http_request_interface & req , callback && cb){
    // TODO
    assert(false);
    return *this;
}
